...
echo "Provisionamento personalizado concluído."#!/bin/bash
...
# Tenta encontrar o caminho de ativação do Conda e ativar o ambiente padrão do ComfyUI
CONDA_BASE_PATH=$(conda info --base 2>/dev/null)
if [ -n "$CONDA_BASE_PATH" ]; then
    source "$CONDA_BASE_PATH"/etc/profile.d/conda.sh # Importa as funções do conda
    echo "Conda base path: $CONDA_BASE_PATH"
    # Tenta ativar o ambiente 'comfy'. Se não existir, lista os ambientes para depuração.
    if conda activate comfy; then
        echo "Ambiente 'comfy' ativado."
    elif conda activate base; then # Fallback para o ambiente base se 'comfy' não existir
        echo "Ambiente 'base' ativado (ambiente 'comfy' não encontrado)."
    else
        echo "Nenhum ambiente Conda detectado ou ativado. Verifique a instalação do Conda."
    fi
...
git config pull.rebase false # Evita problemas de rebase em caso de conflitos
git pull origin master # Garante que puxa da branch master, que é a mais atual
pip install -r requirements.txt --no-cache-dir --upgrade --force-reinstall # Força a reinstalação de dependências
pip install xformers --upgrade # Garante que xformers esteja atualizado
echo "ComfyUI atualizado e dependências forçadas."

# Opcional: Atualizar o frontend do ComfyUI separadamente (pode ser necessário para novas funcionalidades visuais)
# pip install comfyui-frontend-package --upgrade # Pode ser necessário, mas geralmente o git pull já resolve

# --- 2. Instalação de Ferramentas Adicionais (Resto do seu script, inalterado) ---
